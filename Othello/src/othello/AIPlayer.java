...
public class AIPlayer extends Player{
	...
	public synchronized Point getMove(GameState state,GamePanel panel) {
		...
		if(state.getPlayer(2).getNumber()==2) {
			for(Point move: state.getAllLegalMoves(state.getPlayer(2))) {
				int moveScore = getMinScore(state.getPlayer(2),state.playMove(move,state.getPlayer(2)),DEPTH-1);
